...
*/
public class ChangeInCurrency {

	boolean canGetExactChange(int targetMoney, int[] denominations, int index) {
		if (targetMoney == 0)
			return true;
		if (targetMoney > 0 && index < 0)
			return false;
		if (denominations[index] > targetMoney)
			return canGetExactChange(targetMoney, denominations, index - 1);
		else if (denominations[index] < targetMoney) {
			while ((targetMoney -= denominations[index]) > denominations[index])
				;
			return canGetExactChange(targetMoney, denominations, index - 1);
		} else
			return true;
	...
	boolean canGetExactChange(int targetMoney, int[] denominations) {
		return canGetExactChange(targetMoney, denominations, denominations.length - 1);
	}
	
	@Test
	public void test() {
		Assert.assertFalse(canGetExactChange(94, new int[]{5, 10, 25, 100, 200}));
		Assert.assertTrue(canGetExactChange(75, new int[]{4, 17, 29}));
	}
}
