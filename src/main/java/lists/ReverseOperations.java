...
public class ReverseOperations {
	 ...
	 * @return new head - last element in original sequence before the end node
	 */
	Node reverseTillEnd(Node head, Node end) {
		Node curEnd = end;
		while(head.next != end) {
			Node tmp = head.next;
			head.next = curEnd;
	...
	Node reverse(Node head) {
		...
		if(head.data % 2 != 0)
			...
			Node odd = findFirstOdd(head);
			head = reverseTillEnd(head, odd);
			if(odd != null)
				odd.next = reverse(odd.next);
		}
		return head;
	}
